...
class RammpCuroboNode(Node):
    ...
    def __init__(self):
        ...
        self.speed_scale = float(
            self.declare_parameter('speed_scale', 0.0).value)
        self.start_tolerance = float(self.declare_parameter(
            'start_tolerance_rad', 0.05).value)
        self.tracking_tolerance = float(self.declare_parameter(
            'tracking_tolerance_rad', 0.08).value)
        self.gripper_enabled = bool(self.declare_parameter(
            'gripper_enabled', True).value)
        gripper_action = self.declare_parameter(
            'gripper_action', '/robotiq_gripper_controller/gripper_cmd').value
        self.gripper_open_pos = float(
            self.declare_parameter('gripper_open', 0.0).value)
        ...
        self._gripper = ActionClient(
            self, GripperCommand, gripper_action, callback_group=self._cb) \
            if self.gripper_enabled else None

        ActionServer(self, PlanToPose, '~/plan_to_pose',
                     self._plan_to_pose_cb, callback_group=self._cb)
        ActionServer(self, PlanToJoints, '~/plan_to_joints',
                     self._plan_to_joints_cb, callback_group=self._cb)
        ActionServer(self, ExecuteTrajectory, '~/execute_trajectory',
                     self._execute_cb, callback_group=self._cb,
                     cancel_callback=lambda _req: CancelResponse.ACCEPT)
        self.create_service(SetWorld, '~/set_world', self._set_world_cb,
                            callback_group=self._cb)
        if self.gripper_enabled:
            self.create_service(Trigger, '~/open_gripper',
                                self._open_gripper_cb, callback_group=self._cb)
    ...
    def current_q(self, max_age_s=2.0):
        ...
        try:
            return [float(msg.position[idx[n]])
    ...
    def _plan_to_pose_cb(self, goal_handle):
        ...
        quat = [req.orientation.x, req.orientation.y,
                req.orientation.z, req.orientation.w]
        res = self._plan(lambda q: self.planner.plan_to_pose(
            pos, quat, start=q))
    ...
    def _plan_to_joints_cb(self, goal_handle):
        ...
        if len(target) != len(self.planner.joint_names):
            ...
            result.message = ('%d target joints for %d-joint arm'
                              % (len(target), len(self.planner.joint_names)))
            goal_handle.abort()
            return result
        res = self._plan(lambda q: self.planner.plan_to_joints(
            target, start=q))
    ...
    def _finish_plan(self, goal_handle, result, res):
        ...
        if res is False:
            result.success = False
            result.message = ('no fresh joint state on %s — is the arm/sim '
    ...
    def _execute_cb(self, goal_handle):
        ...
        try:
            ...
            if not 0.0 < scale <= self.max_speed_scale:
                return refuse('speed_scale %.3f outside (0, %.2f]'
                              % (scale, self.max_speed_scale))
            q = self.current_q()
            problems = validate_goal_msg(
                msg, self.planner.joint_names, q,
                self._pos_limits, self._vel_limits, self.start_tolerance)
            if problems:
            ...
            status, message = self.executor_helper.run(
                msg, scale, goal_handle=goal_handle, feedback_cb=feedback,
