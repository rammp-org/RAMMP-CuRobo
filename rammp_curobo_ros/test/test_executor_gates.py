...
def test_gate_wrong_joint_names():
    ...
    problems = validate_goal_msg(msg, JOINTS, [0.0] * 7, POS_LIMITS, VEL_LIMITS, 0.05)
...
def test_gate_stale_start():
    ...
    problems = validate_goal_msg(msg, JOINTS, q_now, POS_LIMITS, VEL_LIMITS, 0.05)
    assert any("stale plan" in p for p in problems)


def test_gate_no_joint_state():
    problems = validate_goal_msg(_msg(), JOINTS, None, POS_LIMITS, VEL_LIMITS, 0.05)
...
def test_gate_discontinuity():
    ...
    problems = validate_goal_msg(
        msg, JOINTS, list(msg.points[0].positions), POS_LIMITS, VEL_LIMITS, 0.05
    )
...
def test_scaling_dilates_msg_exactly():
    ...
    assert (
        validate_goal_msg(
            slow, JOINTS, list(msg.points[0].positions), POS_LIMITS, VEL_LIMITS, 0.05
        )
        == []
    )
