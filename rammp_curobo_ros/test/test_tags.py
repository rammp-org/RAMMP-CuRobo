...
def test_detect_recovers_a_synthetic_tag_pose():
    """Render a marker at a known pose, detect it, recover the pose."""
    cv2 = __import__("cv2")
    ...
    assert ids is not None and int(ids.ravel()[0]) == 0
    s = size / 2.0
    objp = np.array([[-s, s, 0], [s, s, 0], [s, -s, 0], [-s, -s, 0]])
    ok, rvec, tvec = cv2.solvePnP(objp, corners[0].reshape(4, 2).astype(float),
                                  k, np.zeros(5),
