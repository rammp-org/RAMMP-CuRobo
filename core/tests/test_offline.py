...
from rammp_curobo.config import (PACKAGED_CONFIG_DIR, load_planner_config,
                                 resolve_config)
from rammp_curobo.geometry import (euler_deg_to_quat_xyzw, spin_about_tool,
                                   tip_to_tool, tool_axis, wxyz_to_xyzw,
                                   xyzw_to_wxyz)
...
def test_packaged_configs_exist():
    for name in ('gen3.yaml', 'robot_gen3_2f85.yaml',
                 'world_sim_kitchen.yaml', 'world_real_bench.yaml'):
...
def test_world_cuboids_padding_and_ignore():
    scene = load_scene(resolve_config('world_sim_kitchen.yaml'))
    boxes = world_cuboids(scene, padding=0.02, ignore={'bottle'},
    ...
    assert boxes['table']['dims'] == pytest.approx(
        [d + 0.04 for d in table.dims])
...
def test_scene_from_obstacle_dicts():
    scene = scene_from_obstacles([
        {'name': 'box', 'position': [0.5, 0.0, 0.0],
         'dims': [0.1, 0.2, 0.3]},
        {'name': 'can', 'type': 'cylinder', 'position': [0.3, 0.1, 0.0],
...
def _traj(n=50, dof=7, dt=0.02, vmax=1.0):
    ...
    return Trajectory(joint_names=['joint_%d' % i for i in range(1, dof + 1)],
                      positions=pos, velocities=vel, accelerations=None,
                      dt=dt)
