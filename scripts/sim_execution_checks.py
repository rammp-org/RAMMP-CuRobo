...
def main():
    ...
    node.create_subscription(
        ...
        lambda m: latest.update(v=max(abs(x) for x in m.velocity[:7]) if m.velocity else None), 10)
    ...
    assert res.status == GoalStatus.STATUS_ABORTED and 'speed_scale' in res.result.message, \
